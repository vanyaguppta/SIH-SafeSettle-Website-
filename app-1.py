...
<div id="app-view" class="app-shell">
  <aside class="sidebar">
    <div class="brand"><span class="dot"></span><div><h1>SAFESETTLE</h1><span>Assam · by Ascendant</span></div></div>
...
const SAMPLE_HAZARD_FEED = [
  {title:'⚠️ River Level Alert (demo)', message:'Brahmaputra level near Jorhat rising steadily — monitor closely.'},
  {title:'🌧️ Weather Alert (demo)', message:'Heavy rainfall forecast over Sivasagar & Charaideo in next 12h.'},
  {title:'📍 Relocation Update (demo)', message:'Golaghat Highland Site now at 82% capacity.'},
  {title:'🍚 Resource Alert (demo)', message:'Cooking oil stock at central store below 15-day buffer.'},
  {title:'🚨 High Risk Update (demo)', message:'Kamrup Metro Wetland Basti risk score revised upward.'},
];
function startNotificationEngine(){
  // Seed with real backend alerts first, then simulate a periodic live feed for the demo.
  getJSON('/api/alerts').then(alerts=>{
    alerts.slice(0,3).forEach(a=>{
      notifStore.push({title:a.type, message:a.title+': '+a.message, time:a.created_at});
    });
    renderNotifDropdown();
  });
  setInterval(()=>{
    const pick = SAMPLE_HAZARD_FEED[Math.floor(Math.random()*SAMPLE_HAZARD_FEED.length)];
    pushNotification({...pick, time:'just now'});
  }, 15000);
}
    ...
    <div class="grid-2">
      ...
      <div class="card"><h3 style="margin-top:0">Supporting Actions</h3>
        ...
        <div style="display:flex;gap:10px;margin-top:18px">
          <button class="btn btn-primary" onclick="window.print()">Download Plan (PDF)</button>
...
async function pageRelocationSites(){
  const sites = await getJSON('/api/habitations/1/sites'); // reuse for markers fallback
  document.getElementById('content').innerHTML = `
    <div class="page-header"><div><h2>Relocation Sites</h2><p>Elevated, safer ground identified across Assam districts</p></div></div>
    <div class="card">${freshMapContainer('map-relsites', 420)}</div>`;
  const map = buildMap('map-relsites', [26.7,94.2], 8);
  const allSites = await getJSON('/api/report/1');
  addSiteMarkers(map, allSites.sites);
}
...
async function pageReports(){
  ...
  document.getElementById('rep-gen').onclick = async () => {
    ...
    document.getElementById('report-area').innerHTML = `
      <div class="card">
        <h3>${h.name} — Full Risk & Relocation Report</h3>
        ...
        ${r.sites.map(s=>`<p>${s.name}: suitability ${s.suitability_score}/100, distance ${s.distance_km} km, capacity ${s.capacity} ${s.recommended?'— RECOMMENDED':''}</p>`).join('')}
        <h4>Recommendation</h4><p>${h.recommendation}</p>
      </div>`;
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div class="card">
      <div class="form-grid">
        ...
        <div><label>Email ID</label><input id="p-email" value="${p.email||''}" placeholder="e.g. name@example.com"/></div>
      ...
      <p style="font-size:12px;color:#667085;margin-top:16px">Stored locally in this app's own SQLite database on your machine — this is a single-user demo profile, not a public account system.</p>
    </div>`;
...
function pageSettings(){
  document.getElementById('content').innerHTML = `
    <div class="page-header"><div><h2>Settings</h2><p>Prototype configuration</p></div></div>
    <div class="card">
